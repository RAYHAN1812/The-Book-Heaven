...
const AllBooks = () => {
    ...
    const sortedBooks = useMemo(() => {
        ...
        if (sortOrder === 'rating_desc') currentBooks.sort((a, b) => b.rating - a.rating);
        if (sortOrder === 'rating_asc') currentBooks.sort((a, b) => a.rating - b.rating);
        return currentBooks;
    }, [rawBooks, sortOrder]);
    ...
    return (
        <div>
            <h1 className="main-heading text-center">All Books 📚</h1>
            
            <div className='flex justify-between items-center mb-6 p-4 bg-base-200 rounded-lg shadow-md'>
                <button onClick={handleSortToggle} className="btn btn-outline btn-primary">
                    <FaStar />
                    Sort by Rating: 
                    {sortOrder === 'rating_desc' && <> High to Low <span className='ml-1'>⬇️</span></>}
                    {sortOrder === 'rating_asc' && <> Low to High <span className='ml-1'>⬆️</span></>}
